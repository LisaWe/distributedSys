 ...
 * handling {@linkplain InterruptedException}; notice the disadvantage of semaphores when it comes
 * to child thread interruption (you'd need to collect the child threads for this). There are two
 * rules which must always be obeyed when handling such an exception:
 * <ul>
...
public final class ResyncThreadBySemaphore2 {
	...
	static private void resync (final int childThreadCount, final int maximumWorkDuration) throws ExampleWorkerException {
		...
		try {
			...
			// kills this thread, but not the child threads. However, because the parent thread dies,
			// the child threads can no longer influence it's course, thereby preserving the before-after
			// relationship between the threads! Note that thread killing must be used with utmost caution; most
			// of the time it's better to kill the whole process instead, as this also kills the child threads!
